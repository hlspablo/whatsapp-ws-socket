import { AuthenticationCreds, AuthenticationState, BufferJSON, initAuthCreds, SignalDataTypeMap, proto } from '@whiskeysockets/baileys'
...
export class AuthStateManager {
    ...
    async getAuthState(
        ...
        const saveState = async (updatedCreds: AuthenticationCreds) => {
            try {
                ...
                await this.databaseManager.updateWhatsappSession(parseInt(sessionId), sessionData)
                ...
                saveInProgress = false
                console.error(`Error saving auth state to database for ${sessionId}:`, error)
            }
        }
                ...
                keys: {
                    ...
                    set: (data: any) => {
                        ...
                        saveState(creds).catch(error => {
                            console.error(`Error in keys.set saveState for ${sessionId}:`, error)
                        })
                    }
                }
            },
    ...
    async deleteAuthState(sessionId: string): Promise<void> {
        try {
            ...
            console.error(`Error deleting auth state from database for ${sessionId}:`, error)
        }
    }
    ...
    async exportAuthState(sessionId: string): Promise<string | null> {
        try {
            ...
            const dbConnection = await this.databaseManager.getWhatsappConnection(parseInt(sessionId))
