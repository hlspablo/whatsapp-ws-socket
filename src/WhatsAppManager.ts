...
export class WhatsAppManager extends EventEmitter {
  ...
  async initializeFromDatabase(): Promise<void> {
    try {
      ...
      const dbConnections =
        await this.databaseManager.getAllWhatsappConnections();
      this.logger.info(
        `Found ${dbConnections.length} WhatsApp connections in database`
      );
      ...
      for (const dbConnection of dbConnections) {
        try {
          ...
          if (dbConnection.session) {
            this.logger.info(
              `Attempting to restore session for ${dbConnection.name} (ID: ${dbConnection.id})`
            );
            ...
            try {
              ...
              this.logger.warn(
                `Failed to restore session for ${dbConnection.name} (ID: ${dbConnection.id}), will create new session:`,
                error
              );
            ...
            this.logger.info(
              `No existing session for ${dbConnection.name} (ID: ${dbConnection.id}), creating new session`
            );
            await this.startConnection(connectionData);
          }
        } catch (error) {
          this.logger.error(
            `Failed to initialize connection for ${dbConnection.name} (ID: ${dbConnection.id}):`,
            error
          );
        }
      }
  ...
  async startConnection(connectionData: StartConnectionData): Promise<{
    ...
    try {
      ...
      if (this.deletedSessions.has(id)) {
        this.logger.info(
          `Session ${id} was deleted, removing from deleted set and allowing new connection`
        );
  ...
  private async reconcileSessionsWithDatabase(): Promise<void> {
    try {
      ...
      const dbIds = new Set<string>(
        dbConnections.map((c: any) => String(c.id))
      );

      for (const sessionId of Array.from(this.sessions.keys())) {
        if (!dbIds.has(sessionId)) {
          this.logger.warn(
            `Reconciliation: session ${sessionId} not found in DB. Deleting in-memory session.`
          );
          try {
            await this.deleteSession(sessionId);
          } catch (deleteError) {
            this.logger.error(
              `Error deleting in-memory session ${sessionId} during reconciliation:`,
              deleteError
            );
          }
        }
  ...
  private async createConnection(
    connectionData: StartConnectionData
  ): Promise<Session> {
    ...
    try {
      this.logger.info("Fetching latest WhatsApp Web version...");
      ...
      const timeoutPromise = new Promise<never>((_, reject) =>
        setTimeout(
          () =>
            reject(
              new Error(
                `fetchLatestBaileysVersion timed out after ${timeoutMs}ms`
              )
            ),
          timeoutMs
        )
      ...
      try {
        ...
        this.logger.warn(
          { err: vfErr instanceof Error ? vfErr.message : vfErr },
          "Failed to fetch latest WA version in time, proceeding without explicit version"
        );
      }

      // Get authentication state
      const { state, saveState, exportStateData } =
        await this.authManager.getAuthState(id, session || undefined);
      ...
      const enhancedSaveState = async () => {
        try {
          ...
          this.logger.error(
            `Error saving enhanced auth state for ${id}:`,
            error
          );
        }
      };
        ...
        wsocket.ev.on("connection.update", async (update) => {
          ...
          if (isNewLogin && connection === "open") {
            ...
            try {
              ...
              this.logger.error(
                `Error saving session after new login for ${id}:`,
                error
              );
            }
          }
          ...
          if (connection === "close") {
            ...
            const isQrAttemptsEnded =
              statusCode === 408 &&
              lastError?.message === "QR refs attempts ended";
            const noRestartErrorCodes = [401, 403];

            this.logger.info(`Connection closed for ${id}`);
            this.logger.info(
              `Disconnect reason: ${lastError?.output?.statusCode}, Error: ${lastError?.message}`
            );
            ...
            if (
              (statusCode && noRestartErrorCodes.includes(statusCode)) ||
              isQrAttemptsEnded
            ) {
              this.logger.warn(
                `No restart needed for ${id}, disconnecting session`
              );
              await this.databaseManager.updateWhatsappStatus(
                parseInt(id),
                "DISCONNECTED"
              );
              ...
              this.logger.warn(
                `Restart needed for ${id}, restarting connection`
              );
              try {
                await this.handleStreamRestart(connectionData);
              } catch (error) {
                this.logger.error(
                  `Error restarting connection for ${id}:`,
                  error
                );
              }
            }
          ...
          if (connection === "open") {
            ...
            try {
              ...
              this.logger.error(
                `Error updating database status for ${id}:`,
                dbError
              );
            }
          ...
          if (qr !== undefined) {
            ...
            try {
              ...
              this.logger.error(
                `Error generating QR code for terminal for ${id}:`,
                qrError
              );
            }
            ...
            try {
              ...
              this.logger.error(
                `Error updating database QR code for ${id}:`,
                dbError
              );
            }
            ...
            for (const message of messages) {
              ...
              if (message.messageStubType === 2) {
                ...
                if (
                  ...
                  this.logger.warn(
                    { stubParams, whatsappId: wsocket.id },
                    "Received session error stub for peer, relying on Baileys retry logic"
                  );
                }
              }
    ...
    try {
      ...
      if (!message.message || message.messageStubType) {
        if (message.messageStubType) {
          this.logger.debug(
            `Ignoring message stub (error) type ${message.messageStubType} for session ${session.id}`
          );
        }
        return;
      }
      ...
      this.logger.info(
        `Processing message from ${message.key.remoteJid}) for session ${session.id}`
      );
      ...
      this.logger.error(
        `Error handling incoming message for session ${session.id}:`,
        error
      );
    }
  }
  ...
  > {
    ...
    try {
      ...
      const mediaProcessingResult =
        await this.mediaProcessor.processMessageMedia(message, onlyAudio);
      ...
      this.logger.debug(
        `About to send message to ${to} via session ${whatsappId}`
      );

      // Send message with properly formatted data
      // to is JID
      const sentMessage = await session.sendMessage(
        to,
      ...
      this.logger.debug(
        `Message sent, checking session state for ${whatsappId}`
      );

      this.logger.info(
        `Message sent successfully to ${to} via session ${whatsappId}`
      );
      return sentMessage;
      ...
      this.logger.error(
        `Error sending message via session ${whatsappId}:`,
        error
      );
      return {
        success: false,
  ...
  async deleteSession(whatsappId: string): Promise<boolean> {
    try {
      this.logger.info(
        `Deleting session ${whatsappId} (removed from database)`
      );
      ...
      if (pendingTimeout) {
        ...
        this.logger.info(
          `Cancelled pending reconnection timeout for session ${whatsappId}`
        );
      }
    ...
    try {
      ...
      if (existingSession) {
        try {
          ...
          this.logger.warn(
            `Error closing existing websocket for ${whatsappId}:`,
            error
          );
        }
  ...
  private async handleStreamRestart(
    connectionData: StartConnectionData
  ): Promise<void> {
    const { id } = connectionData;
    ...
    try {
      ...
      if (currentRetries >= this.maxRetryAttempts) {
        this.logger.error(
          `Max retry attempts (${this.maxRetryAttempts}) reached for session ${id}. Not attempting restart.`
        );
        ...
        try {
          ...
          this.logger.error(
            `Error updating database status after max retries for ${id}:`,
            dbError
          );
        }
      ...
      this.logger.info(
        `Stream error detected for ${id}. Attempting restart in ${delay}ms (attempt ${
          currentRetries + 1
      ...
      const restartTimeout = setTimeout(async () => {
        try {
          ...
          this.logger.info(
            `Starting automatic restart for session ${id} after stream error`
          );
          ...
          if (result.success) {
            this.logger.info(
              `Successfully restarted session ${id} after stream error`
            );
            // Reset retry count on successful connection
            this.retryAttempts.delete(id);
          } else {
            this.logger.warn(
              `Failed to restart session ${id} after stream error: ${result.message}`
            );
            // The retry will be handled by the next connection.update event if it fails again
          }
        } catch (error) {
          this.logger.error(
            `Error during automatic restart for session ${id}:`,
            error
          );
      ...
      this.logger.error(
        `Error setting up stream error restart for ${id}:`,
        error
      );
    }
  }
    ...
    try {
      ...
      if (session) {
        try {
          ...
          this.logger.warn(
            `Error ending session ${sessionId} gracefully:`,
            endError
          );
        }
  ...
  async callSocketMethod(
    ...
    try {
      ...
      if (methodName === "sendMessage") {
        ...
        this.logger.info(
          { sessionId: session.id, jidOrNumber },
          `Routing 'sendMessage' through WhatsAppManager.sendMessage for media processing`
        );
      ...
      if (typeof (session as any)[methodName] !== "function") {
        throw new Error(
          `Method '${methodName}' is not available on WhatsApp socket`
        );
      }

      this.logger.info(
        { methodName, sessionId: session.id, args },
        `Calling socket method '${methodName}' on session ${session.id} with ${args.length} arguments`
      );
    ...
    const disconnectPromises = Array.from(this.sessions.keys()).map(
      async (sessionId) => {
        try {
          ...
          this.logger.error(
            `Error disconnecting session ${sessionId} during shutdown:`,
            error
          );
        }
      }
